...
import { FaEnvelope, FaBookReader, FaUserCircle, FaBars } from "react-icons/fa";
...
const StudentNavbar = ({ toggleSidebar }) => {
  ...
  useEffect(() => {
    const handleClickOutside = (e) => {
      if (!e.target.closest(".account-menu")) setIsAccountOpen(false);
    };
  ...
  return (
    <nav className="bg-[#002147] text-white px-4 sm:px-6 py-3 flex justify-between items-center shadow-md z-50 fixed top-0 left-0 right-0">
      ...
      <div className="flex items-center space-x-3">
        ...
        <img src={logo} alt="University Logo" className="h-10 w-10 sm:h-12 sm:w-12 md:h-16 md:w-16 object-contain" />
      ...
      <div className="flex items-center space-x-4 sm:space-x-6 relative">
        ...
        <div className="hidden lg:flex items-center space-x-6">
          <div className="relative group">
            <Link to="/student/assignments" className="hover:text-gray-300">
          ...
          <div className="relative group">
            <Link to="/student/messages" className="hover:text-gray-300">
              <FaEnvelope size={20} />
            </Link>
            <span className="absolute bottom-[-50px] left-1/2 -translate-x-1/2 text-xs bg-black text-white px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition">
              Messages
        ...
        <div className="relative account-menu border-l-2 pl-4 sm:pl-6 md:pl-10">
          <button
            onClick={(e) => { e.stopPropagation(); setIsAccountOpen((prev) => !prev); }}
      ...
      {isMobileMenuOpen && (
        <div className="absolute top-full right-0 mt-2 w-48 bg-[#002147] border border-gray-700 rounded shadow-lg lg:hidden z-50">
          <Link to="/student/assignments" className="block px-4 py-3 hover:bg-[#001933] flex items-center">
            <FaBookReader className="mr-2" size={16} />
            Assignments
          </Link>
          <Link to="/student/messages" className="block px-4 py-3 hover:bg-[#001933] flex items-center">
